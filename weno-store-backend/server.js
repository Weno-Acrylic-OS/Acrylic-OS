...
app.get('/api/os/latest', (req, res) => {
    ...
    // --- Alyssa App Creation Endpoint ---
    app.post('/api/alyssa/create-app', (req, res) => {
    const { prompt } = req.body;
    if (!prompt) {
        return res.status(400).send('Missing prompt in request body.');
    }

    // Simulate AI creating an app
    console.log(`Alyssa: Received prompt to create app: "${prompt}"`);

    const newApp = {
        id: apps.length > 0 ? Math.max(...apps.map(a => a.id)) + 1 : 1,
        name: `AI App: ${prompt.substring(0, 20)}`, // Truncate name
        version: "1.0.0",
        description: `An app generated by Alyssa based on the prompt: "${prompt}"`,
        packageUrl: '/uploads/ai_generated_app.wfo',
        installed: false
    };

    apps.push(newApp);
    saveDatabase();
    console.log('Alyssa: New app created and added to store:', newApp);
    res.status(201).json({ message: 'App created successfully and is now available in the Weno Store.', app: newApp });
    });

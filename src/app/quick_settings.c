...
static void simulate_fall_event_handler(lv_event_t * e) {
    (void)e; // Unused
    // Hide the quick settings panel before showing the alert
...
void create_quick_settings(lv_obj_t * parent)
{
    ...
    lv_obj_set_grid_cell(btn4, LV_GRID_ALIGN_STRETCH, 3, 1, LV_GRID_ALIGN_CENTER, 0, 1);
    lv_obj_add_event_cb(btn4, simulate_fall_event_handler, LV_EVENT_CLICKED, NULL);
    lv_obj_t * btn_label4 = lv_label_create(btn4);
    lv_label_set_text(btn_label4, LV_SYMBOL_WARNING);
